Public Class Class_Compute
    ...
    Public Sub Compute()
        ...
        Log("Start")
        Log((Simulation.Avancement * 100.0).ToString + "%")
        ...
        For W As Integer = Wmin To Wmax
            ...
            If Simulation.AxeFixe <> Quaternion.QAxe.W Then
                ...
                If Simulation.Avancement <= NbAvancement Then
                    ...
                    Log((NbAvancement * 100.0).ToString + "%")
                    ' Log(W.ToString + "/" + (Simulation.NbPoint - 1).ToString + " = " + NbAvancement.ToString)
            ...
            For X As Integer = Xmin To Xmax
                ...
                If Simulation.AxeFixe <> Quaternion.QAxe.X Then
                    NbAvancement = CDbl(X) / CDbl(Simulation.NbPoint - 1)
                    If Simulation.Avancement <= NbAvancement Then
                        ' SyncLock Simulation
                        Simulation.Avancement = NbAvancement
                        Log((NbAvancement * 100).ToString + "%")
                        '   Log(X.ToString + "/" + (Simulation.NbPoint - 1).ToString + " = " + NbAvancement.ToString)
                        ' End SyncLock
                    End If
                End If
